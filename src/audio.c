...
int PlayMediaFile(char *cPath, int video_enable, int audio_enable)
{
	DBG_LOG_IN();
...
{	
	...
	AVDictionary *stream_opts = 0;
	av_dict_set_int(&stream_opts, "timeout", uiIoTimeout, 0);
...
void* thread_SaveFLVFile(void* pvData)
{
	...
	{
		uiEvent = 0;
		if ((iLevelLoadAudio == 4) || (iLevelLoadVideo == 5))
		...
		//if (ret != 0)
		{
			...
			if ((omx_videobuff.nFilledLen == 0) &&
				((iFileSize >= mBuff->data_size)  
				|| ((MaxDuration <= video_pts) && (iLevelLoadVideo)) 
				|| ((MaxDuration <= audio_pts) && (iLevelLoadAudio))
