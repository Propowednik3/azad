		...
		*uiType = WEB_MSG_EVNTACTIONS;
		...
		if (ret > 0) 
		{
			...
			if (WEB_get_param_from_url(msg, msg_len, "AfterAccept", param_value, 2048) >= 0) 
				iValue[13] = Str2Int(param_value) & 3;
	...
	if (ret == 0) 
	{
		*uiType = WEB_MSG_DIRECTORIES;
		...
		if (ret > 0) 
		{
			...
			if (WEB_get_param_from_url(msg, msg_len, "Access", param_value, 2048) >= 0) 
				iValue[1] = Str2Int(param_value);
			if (WEB_get_param_from_url(msg, msg_len, "RemAcs", param_value, 2048) >= 0) 
				iValue[2] = SearchStrInDataCaseIgn(param_value, strlen(param_value), 0, "ON") ? 1 : 0;
	...
	if (ret == 0) 
	{
		*uiType = WEB_MSG_WIDGETS;
		...
		if (ret > 0) 
		{
			...
			if (WEB_get_param_from_url(msg, msg_len, "SrcCell", param_value, 2048) >= 0) 
				iValue[16] = Str2Int(param_value);			
			if (WEB_get_param_from_url(msg, msg_len, "Coef", param_value, 2048) >= 0) 
				flValue[2] = Str2Float(param_value);			
			if (WEB_get_param_from_url(msg, msg_len, "ShowFrom", param_value, 2048) >= 0) 
				flValue[3] = Str2Float(param_value);			
			if (WEB_get_param_from_url(msg, msg_len, "ShowTo", param_value, 2048) >= 0) 
...
int WEB_control_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session)
{
	...
	char *msg_subbody = (char*)DBG_MALLOC(4096);
	memset(msg_subbody, 0, 4096);
	DBG_MUTEX_LOCK(&system_mutex);
	sprintf(msg_subbody, 	
...
int WEB_menu_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session)
{
	...
	char *msg_subbody = (char*)DBG_MALLOC(16384);
...
{
	int n;
	char *msg_subbody = (char*)DBG_MALLOC(10000);
...
int WEB_evntactions_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session, int iPage, int errcode)
{
			...
			sprintf(msg_subbody,
						...
						n,n, (char*)&maActionInfo[n].ActionID, cDisableFlag, 
							...
							maActionInfo[n].Name, cDisableFlag, 
							 cDisableFlag, ((maActionInfo[n].AfterAccept & 3) == 0) ? "selected" : "", 0,
							(maActionInfo[n].AfterAccept == PA_STOP) ? "selected" : "", PA_STOP,
							(maActionInfo[n].AfterAccept == PA_OFF) ? "selected" : "", PA_OFF,
							(maActionInfo[n].AfterAccept == (PA_STOP | PA_OFF)) ? "selected" : "", PA_STOP | PA_OFF,
	...
	sprintf(msg_subbody, 
					...
					ACTION_TEST_TYPE_EQUALLY, ACTION_TEST_TYPE_LESS, ACTION_TEST_TYPE_MORE, ACTION_TEST_TYPE_AND, ACTION_TEST_TYPE_NOT, 
					ACTION_TEST_TYPE_EQUALLY, ACTION_TEST_TYPE_LESS, ACTION_TEST_TYPE_MORE, ACTION_TEST_TYPE_AND, ACTION_TEST_TYPE_NOT,
					0, PA_STOP, PA_OFF, PA_STOP | PA_OFF, iSubPages - 1);		
	strcat(msg_tx, msg_subbody);
	DBG_MUTEX_UNLOCK(&evntaction_mutex);
...
int WEB_directory_save(int *pParams, char* strParam)
{
	...
	if ((pParams[0] >= 0) && (pParams[0] < iDirsCnt) && iDirsCnt)
	{
		...
		if (strlen(&strParam[64]) > 255) memcpy(diDirList[pParams[0]].Path, &strParam[64], 255);
			else memcpy(diDirList[pParams[0]].Path, &strParam[64], strlen(&strParam[64]));
		while (diDirList[pParams[0]].Path[strlen(diDirList[pParams[0]].Path)-1] == 47) 
							diDirList[pParams[0]].Path[strlen(diDirList[pParams[0]].Path)-1] = 0;
		ret = 1;
	}
...
int WEB_directories_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session, int iPage, int errcode)
{
	...
	strcat(msg_tx, "<table border='1' width='100%' cellpadding='5'>"
					"<tr><th>Номер</th><th>Доступ</th><th>Удаленный доступ</th><th>Наименование</th><th>Путь</th><th>Действие</th></tr>");
	...
	for (n = iFirstOption; n < iLastOption; n++)
	{
		...
		sprintf(msg_subbody, 
					...
					"<td><input type='number' name='Pp' value=%i style='width: 50px;' disabled></td>\r\n"
					"<td><input type='number' name='Access' min=0 max=1000 value=%i style='width: 60px;'></td>\r\n"
					"<td><input type='checkbox' name='RemAcs' %s></td>\r\n"
					"<td><input type='text' name='Name' maxlength=64 value='%s' style='width: 300px;'></td>\r\n"
	...
	sprintf(msg_subbody, 
					...
					"<input type='hidden' name='req_index' value=%i>\r\n"
					"<td><input type='number' name='Access' min=0 max=1000 value=0 style='width: 60px;'></td>\r\n"
					"<td><input type='checkbox' name='RemAcs'></td>\r\n"
					"<td><input type='text' name='Name' maxlength=64 value='' style='width: 300px;'></td>\r\n"
					"<td><input type='text' name='Path' maxlength=255 value='' style='width: 360px;'></td>\r\n"
					"<input type='hidden' name='Page' value=%i>"
					"<td><button type='submit'>Добавить</button></td></form></tr>\r\n", 
					session->request_id,
					iSubPages - 1);		
...
int WEB_widget_add(int *pParams, char* strParam, float* flParams)
{
	...
	if (pParams[20] < (iWidgetsCnt-1)) WEB_change_pos(iWidgetsCnt-1, pParams[20], (char*)wiWidgetList, iWidgetsCnt, sizeof(WIDGET_INFO));
...
int WEB_widget_save(int *pParams, char* strParam, float* flParams)
{
	...
	if ((pParams[0] >= 0) && (pParams[0] < iWidgetsCnt) && iWidgetsCnt)
	{
		...
		if (pParams[0] != pParams[20]) WEB_change_pos(pParams[0], pParams[20], (char*)wiWidgetList, iWidgetsCnt, sizeof(WIDGET_INFO));		
		ret = 1;
	}
	DBG_MUTEX_UNLOCK(&widget_mutex);
	if (ret) SaveWidgets();
...
int WEB_widgets_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session, int iPage, int errcode)
{
	...
	strcat(msg_tx, "<table border='1' width='100%' cellpadding='5'>"
					"<tr><th>Номер</th><th>Включен</th><th>Статус</th><th>Идентификатор</th><th>Режим работы</th><th>Тип</th><th>Наименование</th><th>Масштаб</th>"
						"<th>Направление движения</th><th>Скорость</th><th>День (Weather)</th><th>Период перезагрузки (сек.)</th>"
						"<th>Дни недели</th><th>Интервал времени 1</th><th>Интервал времени 2</th><th>Интервал времени 3</th>"
						"<th>ИД</th><th>Номер сенсора</th><th>Цвет</th><th>Смещение значения</th><th>Коэффициент</th><th>Показывать сенсор С</th><th>Показывать сенсор ПО</th>"
						"<th>Единицы измерения</th><th>Собственная траектория движения</th>"
						"<th>Путь</th><th>Действие</th></tr>");
	...
	for (n = iFirstOption; n < iLastOption; n++)
	{
		...
		sprintf(msg_subbody, 
					...
					"<td> По расписанию:<input type='checkbox' style='width: 150px;' name='SM01'%s%s>\r\n"
						"<br>Меню:<input type='checkbox' name='SM02'%s%s>\r\n"
						"<br>Таймер:<input type='checkbox' name='SM03'%s%s>\r\n"
							" <input type='number' name='SM03_Time' value=%i style='width: 50px;' %s>(%i)сек.\r\n"
							" Повтор:<input type='checkbox' name='Repeat'%s%s>\r\n"
						"<br>Всегда:<input type='checkbox' name='SM04'%s%s>\r\n"
						"</td>\r\n"
					...
					"<td><input type='text' name='SrcID' value='%.4s' maxlength=4 style='width: 60px;'%s></td>\r\n"
					"<td><input type='number' name='SrcCell' min=1 max=%i value=%i%s></td>\r\n"
					...
					"<td><input type='number' name='ShowFrom' step=0.0001 value=%g  style='width: 60px;' %s></td>\r\n"
					"<td><input type='number' name='ShowTo' step=0.0001 value=%g  style='width: 60px;' %s></td>\r\n"
	...
	sprintf(msg_subbody, 
					...
					"<td> По расписанию:<input type='checkbox' name='SM01'>\r\n"
						"<br />Меню:<input type='checkbox' name='SM02'>\r\n"
						"<br />Таймер:<input type='checkbox' name='SM03'>\r\n"
							" <input type='number' name='SM03_Time' value=0 style='width: 50px;'>сек.<br />\r\n"
							" Повтор:<input type='checkbox' name='Repeat'>\r\n"
						"<br />Всегда:<input type='checkbox' name='SM04'>\r\n"
						"</td>\r\n"
					...
					"<td><input type='text' name='SrcID' value='' maxlength=4 style='width: 60px;'></td>\r\n"
					"<td><input type='number' name='SrcCell' min=1 max=%i value=1></td>\r\n"
					...
					"<td><input type='number' name='ShowFrom' step=0.0001 style='width: 60px;'></td>\r\n"
					"<td><input type='number' name='ShowTo' step=0.0001 style='width: 60px;'></td>\r\n"
...
int WEB_streams_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session, int iPage, int errcode)
{
	...
	strcat(msg_tx, "<table border='1' width='100%' cellpadding='5'>"
					"<tr><th>Номер</th><th>Уровень доступа</th><th>Тип</th><th>Наименование</th><th>Путь(Адрес)</th><th>Действие</th></tr>");
...
int WEB_settings_save(int *pParams, char *strParams)
{
	...
	switch(pParams[0])
	{
		...
		case 6:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeFull = Str2Int(&strParams[512]);
			if ((ucBackUpModeFull < 0) || (ucBackUpModeFull > 2)) ucBackUpModeFull = 0;
			ucBackUpOrderLmtFull = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtFull < 0) || (ucBackUpOrderLmtFull > 20)) ucBackUpOrderLmtFull = 0;
			ucBackUpOrderEnableFull = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrFull, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrFull, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrFull, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathFull, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathFull, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathFull, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclFull = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessFull = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;		
		case 7:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeSlow = Str2Int(&strParams[512]);
			if ((ucBackUpModeSlow < 0) || (ucBackUpModeSlow > 2)) ucBackUpModeSlow = 0;
			ucBackUpOrderLmtSlow = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtSlow < 0) || (ucBackUpOrderLmtSlow > 20)) ucBackUpOrderLmtSlow = 0;
			ucBackUpOrderEnableSlow = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrSlow, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrSlow, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrSlow, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathSlow, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathSlow, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathSlow, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclSlow = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessSlow = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 8:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeDiff = Str2Int(&strParams[512]);
			if ((ucBackUpModeDiff < 0) || (ucBackUpModeDiff > 2)) ucBackUpModeDiff = 0;
			ucBackUpOrderLmtDiff = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtDiff < 0) || (ucBackUpOrderLmtDiff > 20)) ucBackUpOrderLmtDiff = 0;
			ucBackUpOrderEnableDiff = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrDiff, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrDiff, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrDiff, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathDiff, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathDiff, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathDiff, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclDiff = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessDiff = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 9:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeAudio = Str2Int(&strParams[512]);
			if ((ucBackUpModeAudio < 0) || (ucBackUpModeAudio > 2)) ucBackUpModeAudio = 0;
			ucBackUpOrderLmtAudio = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtAudio < 0) || (ucBackUpOrderLmtAudio > 20)) ucBackUpOrderLmtAudio = 0;
			ucBackUpOrderEnableAudio = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrAudio, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrAudio, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrAudio, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathAudio, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathAudio, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathAudio, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclAudio = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessAudio = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 10:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeStatuses = Str2Int(&strParams[512]);
			if ((ucBackUpModeStatuses < 0) || (ucBackUpModeStatuses > 2)) ucBackUpModeStatuses = 0;
			ucBackUpOrderLmtStatuses = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtStatuses < 0) || (ucBackUpOrderLmtStatuses > 20)) ucBackUpOrderLmtStatuses = 0;
			ucBackUpOrderEnableStatuses = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrStatuses, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrStatuses, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrStatuses, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathStatuses, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathStatuses, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathStatuses, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclStatuses = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessStatuses = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 11:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeEvents = Str2Int(&strParams[512]);
			if ((ucBackUpModeEvents < 0) || (ucBackUpModeEvents > 2)) ucBackUpModeEvents = 0;
			ucBackUpOrderLmtEvents = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtEvents < 0) || (ucBackUpOrderLmtEvents > 20)) ucBackUpOrderLmtEvents = 0;
			ucBackUpOrderEnableEvents = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrEvents, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrEvents, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrEvents, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathEvents, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathEvents, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathEvents, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclEvents = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessEvents = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 12:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeActions = Str2Int(&strParams[512]);
			if ((ucBackUpModeActions < 0) || (ucBackUpModeActions > 2)) ucBackUpModeActions = 0;
			ucBackUpOrderLmtActions = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtActions < 0) || (ucBackUpOrderLmtActions > 20)) ucBackUpOrderLmtActions = 0;
			ucBackUpOrderEnableActions = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrActions, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrActions, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrActions, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathActions, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathActions, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathActions, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclActions = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessActions = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 13:
			DBG_MUTEX_LOCK(&system_mutex);				
			ucBackUpModeArchive = Str2Int(&strParams[512]);
			if ((ucBackUpModeArchive < 0) || (ucBackUpModeArchive > 2)) ucBackUpModeArchive = 0;
			ucBackUpOrderLmtArchive = Str2Int(&strParams[768]);
			if ((ucBackUpOrderLmtArchive < 0) || (ucBackUpOrderLmtArchive > 20)) ucBackUpOrderLmtArchive = 0;
			ucBackUpOrderEnableArchive = SearchStrInDataCaseIgn(&strParams[1024], 10, 0, "ON") ? 1 : 0;			
			memset(ucBackUpOrderAddrArchive, 0, 32);
			if (strlen(&strParams[1280]) < 32) memcpy(ucBackUpOrderAddrArchive, &strParams[1280], strlen(&strParams[1280]));
											else memcpy(ucBackUpOrderAddrArchive, &strParams[1280], MAX_PATH - 1);
			memset(ucCaptureStartPathArchive, 0, MAX_PATH);
			if (strlen(&strParams[1536]) < MAX_PATH) memcpy(ucCaptureStartPathArchive, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(ucCaptureStartPathArchive, &strParams[1536], MAX_PATH - 1);
			uiBackUpOrderWaitCnclArchive = Str2Int(&strParams[0]);
			uiBackUpOrderWaitMessArchive = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 14:		
			DBG_MUTEX_LOCK(&system_mutex);				
		...
		case 15:
			...
			if ((uiMediaIoTimeout < 1) || (uiMediaIoTimeout > 1000)) uiMediaIoTimeout = 60;
		...
		case 19:
			...
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathFull, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathFull, &strParams[0], 255);
			uiCaptureFileSizeFull = Str2Int(&strParams[256]);
			uiCaptureTimeLimitFull = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathFull, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathFull, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathFull, &strParams[768], 255);
			ucMediaArchiveModeFull = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathFull, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathFull, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathFull, &strParams[1536], 255);
			uiCaptureMinFreeSpaceFull = Str2Int(&strParams[1024]);	
			
			memset(ucCaptureStartPathFull, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathFull, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathFull, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 20:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathSlow, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathSlow, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathSlow, &strParams[0], 255);
			uiCaptureFileSizeSlow = Str2Int(&strParams[256]);
			uiCaptureTimeLimitSlow = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathSlow, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathSlow, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathSlow, &strParams[768], 255);
			ucMediaArchiveModeSlow = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathSlow, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathSlow, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathSlow, &strParams[1536], 255);
			uiCaptureMinFreeSpaceSlow = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathSlow, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathSlow, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathSlow, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 21:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathDiff, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathDiff, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathDiff, &strParams[0], 255);
			uiCaptureFileSizeDiff = Str2Int(&strParams[256]);
			uiCaptureTimeLimitDiff = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathDiff, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathDiff, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathDiff, &strParams[768], 255);
			ucMediaArchiveModeDiff = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathDiff, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathDiff, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathDiff, &strParams[1536], 255);
			uiCaptureMinFreeSpaceDiff = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathDiff, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathDiff, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathDiff, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 22:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathAudio, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathAudio, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathAudio, &strParams[0], 255);
			uiCaptureFileSizeAudio = Str2Int(&strParams[256]);
			uiCaptureTimeLimitAudio = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathAudio, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathAudio, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathAudio, &strParams[768], 255);
			ucMediaArchiveModeAudio = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathAudio, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathAudio, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathAudio, &strParams[1536], 255);
			uiCaptureMinFreeSpaceAudio = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathAudio, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathAudio, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathAudio, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 23:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathStatuses, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathStatuses, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathStatuses, &strParams[0], 255);
			uiCaptureFileSizeStatuses = Str2Int(&strParams[256]);
			uiCaptureTimeLimitStatuses = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathStatuses, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathStatuses, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathStatuses, &strParams[768], 255);
			ucMediaArchiveModeStatuses = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathStatuses, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathStatuses, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathStatuses, &strParams[1536], 255);
			uiCaptureMinFreeSpaceStatuses = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathStatuses, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathStatuses, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathStatuses, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);											
			break;
		case 24:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathEvents, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathEvents, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathEvents, &strParams[0], 255);
			uiCaptureFileSizeEvents = Str2Int(&strParams[256]);
			uiCaptureTimeLimitEvents = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathEvents, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathEvents, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathEvents, &strParams[768], 255);
			ucMediaArchiveModeEvents = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathEvents, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathEvents, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathEvents, &strParams[1536], 255);
			uiCaptureMinFreeSpaceEvents = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathEvents, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathEvents, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathEvents, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;
		case 25:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cMediaCapturePathActions, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cMediaCapturePathActions, &strParams[0], strlen(&strParams[0]));
											else memcpy(cMediaCapturePathActions, &strParams[0], 255);
			uiCaptureFileSizeActions = Str2Int(&strParams[256]);
			uiCaptureTimeLimitActions = Str2Int(&strParams[512]);
			
			memset(cMediaBackUpPathActions, 0, 256);
			if (strlen(&strParams[768]) < 256) memcpy(cMediaBackUpPathActions, &strParams[768], strlen(&strParams[768]));
											else memcpy(cMediaBackUpPathActions, &strParams[768], 255);
			ucMediaArchiveModeActions = Str2Int(&strParams[1280]);
			memset(cMediaArchivePathActions, 0, 256);
			if (strlen(&strParams[1536]) < 256) memcpy(cMediaArchivePathActions, &strParams[1536], strlen(&strParams[1536]));
											else memcpy(cMediaArchivePathActions, &strParams[1536], 255);
			uiCaptureMinFreeSpaceActions = Str2Int(&strParams[1024]);	
			memset(ucCaptureStartPathActions, 0, 256);
			if (strlen(&strParams[1792]) < 256) memcpy(ucCaptureStartPathActions, &strParams[1792], strlen(&strParams[1792]));
											else memcpy(ucCaptureStartPathActions, &strParams[1792], 255);
			DBG_MUTEX_UNLOCK(&system_mutex);											
			break;
		case 26:
			DBG_MUTEX_LOCK(&system_mutex);	
			ucMediaArchiveTimeFromFull = Str2Int(&strParams[0]);
			ucMediaArchiveTimeToFull = Str2Int(&strParams[256]);
			ucMediaArchiveTimeFromSlow = Str2Int(&strParams[512]);
			ucMediaArchiveTimeToSlow = Str2Int(&strParams[768]);
			ucMediaArchiveTimeFromDiff = Str2Int(&strParams[1024]);
			ucMediaArchiveTimeToDiff = Str2Int(&strParams[1280]);
			ucMediaArchiveTimeFromAudio = Str2Int(&strParams[1536]);
			ucMediaArchiveTimeToAudio = Str2Int(&strParams[1792]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;	
		case 27:
			DBG_MUTEX_LOCK(&system_mutex);		
			ucMediaArchiveTimeFromStatuses = Str2Int(&strParams[0]);
			ucMediaArchiveTimeToStatuses = Str2Int(&strParams[256]);
			ucMediaArchiveTimeFromEvents = Str2Int(&strParams[512]);
			ucMediaArchiveTimeToEvents = Str2Int(&strParams[768]);
			ucMediaArchiveTimeFromActions = Str2Int(&strParams[1024]);
			ucMediaArchiveTimeToActions = Str2Int(&strParams[1280]);
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;	
		case 28:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathFull, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathFull, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathFull, &strParams[0], 255);
			memset(cLocalArchivePathFull, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathFull, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathFull, &strParams[256], 255);
			uiBackUpMinFreeSpaceFull = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceFull = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 29:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathSlow, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathSlow, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathSlow, &strParams[0], 255);
			memset(cLocalArchivePathSlow, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathSlow, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathSlow, &strParams[256], 255);
			uiBackUpMinFreeSpaceSlow = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceSlow = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 30:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathDiff, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathDiff, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathDiff, &strParams[0], 255);
			memset(cLocalArchivePathDiff, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathDiff, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathDiff, &strParams[256], 255);
			uiBackUpMinFreeSpaceDiff = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceDiff = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 31:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathAudio, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathAudio, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathAudio, &strParams[0], 255);
			memset(cLocalArchivePathAudio, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathAudio, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathAudio, &strParams[256], 255);
			uiBackUpMinFreeSpaceAudio = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceAudio = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 32:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathStatuses, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathStatuses, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathStatuses, &strParams[0], 255);
			memset(cLocalArchivePathStatuses, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathStatuses, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathStatuses, &strParams[256], 255);
			uiBackUpMinFreeSpaceStatuses = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceStatuses = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 33:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathEvents, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathEvents, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathEvents, &strParams[0], 255);
			memset(cLocalArchivePathEvents, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathEvents, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathEvents, &strParams[256], 255);
			uiBackUpMinFreeSpaceEvents = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceEvents = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 34:
			DBG_MUTEX_LOCK(&system_mutex);			
			memset(cLocalBackUpPathActions, 0, 256);
			if (strlen(&strParams[0]) < 256) memcpy(cLocalBackUpPathActions, &strParams[0], strlen(&strParams[0]));
											else memcpy(cLocalBackUpPathActions, &strParams[0], 255);
			memset(cLocalArchivePathActions, 0, 256);
			if (strlen(&strParams[256]) < 256) memcpy(cLocalArchivePathActions, &strParams[256], strlen(&strParams[256]));
											else memcpy(cLocalArchivePathActions, &strParams[256], 255);
			uiBackUpMinFreeSpaceActions = Str2Int(&strParams[512]);	
			uiArchiveMinFreeSpaceActions = Str2Int(&strParams[768]);	
			DBG_MUTEX_UNLOCK(&system_mutex);
			break;			
		case 35:
			DBG_MUTEX_LOCK(&system_mutex);			
...
int WEB_settings_respond(char *msg_rx, char *msg_tx, WEB_SESSION *session, int iPage, int errcode)
{
	...
	sprintf(msg_subbody, 
				...
				"<tr bgcolor='PapayaWhip'><td>* Записывать события</td><td><input type='checkbox' name='Val4'%s></td></tr>\r\n"
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='Peru'><td>Настройки копирования НЕПРЕРЫВНОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='Peru'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
						"<option %s value='2'>Переместить</option>\r\n"
						"</select></td></tr>\r\n"
				"<tr bgcolor='Peru'><td>* Макс. длина очереди:</td><td><input type='number' name='Val4' min=0 max=30 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='Peru'><td>* Использовать сервис:</td><td><input type='checkbox' name='Val5'%s></td></tr>\r\n"
				"<tr bgcolor='Peru'><td>* IP адрес сервера:</td><td><input type='text' name='Val6' maxlength=15 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='Peru'><td>* Макс. время ожидания до отмены(сек.):</td><td><input type='number' name='Val1' min=10 max=10000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='Peru'><td>* Макс. время ожидания до оповещения(сек.):</td><td><input type='number' name='Val8' min=10 max=10000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt,
				(ucBackUpModeFull == 0) ? " selected " : "", 
				(ucBackUpModeFull == 1) ? " selected " : "", 
				(ucBackUpModeFull == 2) ? " selected " : "",				
				ucBackUpOrderLmtFull,
				ucBackUpOrderEnableFull ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#a76c33'><td>Настройки копирования МЕДЛЕННОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#a76c33'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
						"<option %s value='2'>Переместить</option>\r\n"
						"</select></td></tr>\r\n"
				"<tr bgcolor='#a76c33'><td>* Макс. длина очереди:</td><td><input type='number' name='Val4' min=0 max=30 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#a76c33'><td>* Использовать сервис:</td><td><input type='checkbox' name='Val5'%s></td></tr>\r\n"
				"<tr bgcolor='#a76c33'><td>* IP адрес сервера:</td><td><input type='text' name='Val6' maxlength=15 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#a76c33'><td>* Макс. время ожидания до отмены(сек.):</td><td><input type='number' name='Val1' min=10 max=10000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#a76c33'><td>* Макс. время ожидания до оповещения(сек.):</td><td><input type='number' name='Val8' min=10 max=10000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt,
				(ucBackUpModeSlow == 0) ? " selected " : "", 
				(ucBackUpModeSlow == 1) ? " selected " : "", 
				(ucBackUpModeSlow == 2) ? " selected " : "",				
				ucBackUpOrderLmtSlow,
				ucBackUpOrderEnableSlow ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='Peru'><td>Настройки копирования МЕНЯЮЩЕЙСЯ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='Peru'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeDiff == 0) ? " selected " : "", 
				(ucBackUpModeDiff == 1) ? " selected " : "", 
				(ucBackUpModeDiff == 2) ? " selected " : "",				
				ucBackUpOrderLmtDiff,
				ucBackUpOrderEnableDiff ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#a76c33'><td>Настройки копирования АУДИО записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#a76c33'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeAudio == 0) ? " selected " : "", 
				(ucBackUpModeAudio == 1) ? " selected " : "", 
				(ucBackUpModeAudio == 2) ? " selected " : "",				
				ucBackUpOrderLmtAudio,
				ucBackUpOrderEnableAudio ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='Peru'><td>Настройки копирования записей СТАТУСОВ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='Peru'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeStatuses == 0) ? " selected " : "", 
				(ucBackUpModeStatuses == 1) ? " selected " : "", 
				(ucBackUpModeStatuses == 2) ? " selected " : "",				
				ucBackUpOrderLmtStatuses,
				ucBackUpOrderEnableStatuses ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#a76c33'><td>Настройки копирования записей СОБЫТИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#a76c33'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeEvents == 0) ? " selected " : "", 
				(ucBackUpModeEvents == 1) ? " selected " : "", 
				(ucBackUpModeEvents == 2) ? " selected " : "",				
				ucBackUpOrderLmtEvents,
				ucBackUpOrderEnableEvents ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='Peru'><td>Настройки копирования записей ДЕЙСТВИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='Peru'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeActions == 0) ? " selected " : "", 
				(ucBackUpModeActions == 1) ? " selected " : "", 
				(ucBackUpModeActions == 2) ? " selected " : "",				
				ucBackUpOrderLmtActions,
				ucBackUpOrderEnableActions ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#a76c33'><td>Настройки копирования в АРХИВ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#a76c33'><td>* Режим копирования:</td>"
					"<td><select name='Val3' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Копировать</option>\r\n"
				...
				session->request_id,
				cnt,
				(ucBackUpModeArchive == 0) ? " selected " : "", 
				(ucBackUpModeArchive == 1) ? " selected " : "", 
				(ucBackUpModeArchive == 2) ? " selected " : "",				
				ucBackUpOrderLmtArchive,
				ucBackUpOrderEnableArchive ? " checked " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки НЕПРЕРЫВНОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Размер файла (MB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам резервных копий:</td><td><input type='text' name='Val4' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для захвата (MB):</td><td><input type='number' name='Val5' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Режим копирования в архив:</td>"
					"<td><select name='Val6' style='width: 300;'>\r\n"
						...
						"</select></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Путь к файлам архива (полный для SMB или добавочный для сервиса копирования):</td><td><input type='text' name='Val7' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt, 
				cMediaCapturePathFull, ucCaptureStartPathFull, uiCaptureFileSizeFull, uiCaptureTimeLimitFull, cMediaBackUpPathFull, uiCaptureMinFreeSpaceFull, 
				(ucMediaArchiveModeFull == 0) ? " selected " : "", 
				(ucMediaArchiveModeFull == 1) ? " selected " : "", 
				(ucMediaArchiveModeFull == 2) ? " selected " : "",
				(ucMediaArchiveModeFull == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки МЕДЛЕННОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Размер файла (MB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам резервных копий:</td><td><input type='text' name='Val4' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для захвата (MB):</td><td><input type='number' name='Val5' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Режим копирования в архив:</td>"
					"<td><select name='Val6' style='width: 300;'>\r\n"
						"<option %s value='0'>Отключено</option>\r\n"
						"<option %s value='1'>Случайный</option>\r\n"
						"<option %s value='2'>Макс. размер</option>\r\n"
						"<option %s value='3'>Мин. размер</option>\r\n"
						"</select></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Путь к файлам архива (полный для SMB или добавочный для сервиса копирования):</td><td><input type='text' name='Val7' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt, 
				cMediaCapturePathSlow, ucCaptureStartPathSlow, uiCaptureFileSizeSlow, uiCaptureTimeLimitSlow, cMediaBackUpPathSlow, uiCaptureMinFreeSpaceSlow,
				(ucMediaArchiveModeSlow == 0) ? " selected " : "", 
				(ucMediaArchiveModeSlow == 1) ? " selected " : "", 
				(ucMediaArchiveModeSlow == 2) ? " selected " : "",
				(ucMediaArchiveModeSlow == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки записи ДВИЖЕНИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Размер файла (MB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				...
				session->request_id,
				cnt, 
				cMediaCapturePathDiff, ucCaptureStartPathDiff, uiCaptureFileSizeDiff, uiCaptureTimeLimitDiff, cMediaBackUpPathDiff, uiCaptureMinFreeSpaceDiff,
				(ucMediaArchiveModeDiff == 0) ? " selected " : "", 
				(ucMediaArchiveModeDiff == 1) ? " selected " : "", 
				(ucMediaArchiveModeDiff == 2) ? " selected " : "",
				(ucMediaArchiveModeDiff == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки АУДИО записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Размер файла (MB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				...
				session->request_id,
				cnt, 
				cMediaCapturePathAudio, ucCaptureStartPathAudio, uiCaptureFileSizeAudio, uiCaptureTimeLimitAudio, cMediaBackUpPathAudio, uiCaptureMinFreeSpaceAudio,
				(ucMediaArchiveModeAudio == 0) ? " selected " : "", 
				(ucMediaArchiveModeAudio == 1) ? " selected " : "", 
				(ucMediaArchiveModeAudio == 2) ? " selected " : "",
				(ucMediaArchiveModeAudio == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки записи СОСТОЯНИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Размер файла (KB):</td><td><input type='number' name='Val2' min=10 max=100000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=100000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам резервных копий:</td><td><input type='text' name='Val4' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для захвата (MB):</td><td><input type='number' name='Val5' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Режим копирования в архив:</td>"
					"<td><select name='Val6' style='width: 300;'>\r\n"
				...
				session->request_id,
				cnt, 
				cMediaCapturePathStatuses, ucCaptureStartPathStatuses, uiCaptureFileSizeStatuses, uiCaptureTimeLimitStatuses, cMediaBackUpPathStatuses, uiCaptureMinFreeSpaceStatuses,
				(ucMediaArchiveModeStatuses == 0) ? " selected " : "", 
				(ucMediaArchiveModeStatuses == 1) ? " selected " : "", 
				(ucMediaArchiveModeStatuses == 2) ? " selected " : "",
				(ucMediaArchiveModeStatuses == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки записи СОБЫТИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Размер файла (KB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам резервных копий:</td><td><input type='text' name='Val4' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для захвата (MB):</td><td><input type='number' name='Val5' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Режим копирования в архив:</td>"
				...
				session->request_id,
				cnt, 
				cMediaCapturePathEvents, ucCaptureStartPathEvents, uiCaptureFileSizeEvents, uiCaptureTimeLimitEvents, cMediaBackUpPathEvents, uiCaptureMinFreeSpaceEvents,
				(ucMediaArchiveModeEvents == 0) ? " selected " : "", 
				(ucMediaArchiveModeEvents == 1) ? " selected " : "", 
				(ucMediaArchiveModeEvents == 2) ? " selected " : "",
				(ucMediaArchiveModeEvents == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки записи ДЕЙСТВИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам захвата:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Начало добавочного пути к файлу:</td><td><input type='text' name='Val8' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Размер файла (KB):</td><td><input type='number' name='Val2' min=10 max=1000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Время файла (сек.):</td><td><input type='number' name='Val3' min=60 max=10000 value='%i' style='width: 300px;'></td>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам резервных копий:</td><td><input type='text' name='Val4' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для захвата (MB):</td><td><input type='number' name='Val5' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Режим копирования в архив:</td>"
				...
				session->request_id,
				cnt,
				cMediaCapturePathActions, ucCaptureStartPathActions, uiCaptureFileSizeActions, uiCaptureTimeLimitActions, cMediaBackUpPathActions, uiCaptureMinFreeSpaceActions,
				(ucMediaArchiveModeActions == 0) ? " selected " : "", 
				(ucMediaArchiveModeActions == 1) ? " selected " : "", 
				(ucMediaArchiveModeActions == 2) ? " selected " : "",
				(ucMediaArchiveModeActions == 3) ? " selected " : "",
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Интервалы копирования в АРХИВ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>Непрерывная:</td><td><input type='time' name='Val1' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val2' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Медленная:</td><td><input type='time' name='Val3' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val4' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Движения:</td><td><input type='time' name='Val5' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val6' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>Аудио:</td><td><input type='time' name='Val7' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val8' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt,
				(int)(ucMediaArchiveTimeFromFull / 100), ucMediaArchiveTimeFromFull - (int)(ucMediaArchiveTimeFromFull / 100)*100, 
				(int)(ucMediaArchiveTimeToFull / 100), ucMediaArchiveTimeToFull - (int)(ucMediaArchiveTimeToFull / 100)*100,
				(int)(ucMediaArchiveTimeFromSlow / 100), ucMediaArchiveTimeFromSlow - (int)(ucMediaArchiveTimeFromSlow / 100)*100,
				(int)(ucMediaArchiveTimeToSlow / 100), ucMediaArchiveTimeToSlow - (int)(ucMediaArchiveTimeToSlow / 100)*100,
				(int)(ucMediaArchiveTimeFromDiff / 100), ucMediaArchiveTimeFromDiff - (int)(ucMediaArchiveTimeFromDiff / 100)*100,
				(int)(ucMediaArchiveTimeToDiff / 100), ucMediaArchiveTimeToDiff - (int)(ucMediaArchiveTimeToDiff / 100)*100,
				(int)(ucMediaArchiveTimeFromAudio / 100), ucMediaArchiveTimeFromAudio - (int)(ucMediaArchiveTimeFromAudio / 100)*100,
				(int)(ucMediaArchiveTimeToAudio / 100), ucMediaArchiveTimeToAudio - (int)(ucMediaArchiveTimeToAudio / 100)*100);
	strcat(msg_tx, msg_subbody);
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Состояния:</td><td><input type='time' name='Val1' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val2' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td><td><button type='submit'>Сохранить</button></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>События:</td><td><input type='time' name='Val3' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val4' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Действия:</td><td><input type='time' name='Val5' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'>"
						" по <input type='time' name='Val6' min='00:00' max='23:59' value='%.2i:%.2i' style='width: 100px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt,
				(int)(ucMediaArchiveTimeFromStatuses / 100), ucMediaArchiveTimeFromStatuses - (int)(ucMediaArchiveTimeFromStatuses / 100)*100, 
				(int)(ucMediaArchiveTimeToStatuses / 100), ucMediaArchiveTimeToStatuses - (int)(ucMediaArchiveTimeToStatuses / 100)*100, 
				(int)(ucMediaArchiveTimeFromEvents / 100), ucMediaArchiveTimeFromEvents - (int)(ucMediaArchiveTimeFromEvents / 100)*100, 
				(int)(ucMediaArchiveTimeToEvents / 100), ucMediaArchiveTimeToEvents - (int)(ucMediaArchiveTimeToEvents / 100)*100, 
				(int)(ucMediaArchiveTimeFromActions / 100), ucMediaArchiveTimeFromActions - (int)(ucMediaArchiveTimeFromActions / 100)*100, 
				(int)(ucMediaArchiveTimeToActions / 100), ucMediaArchiveTimeToActions - (int)(ucMediaArchiveTimeToActions / 100)*100);
	strcat(msg_tx, msg_subbody);
	...
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки ХРАНИЛИЩА НЕПРЕРЫВНОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt, 
				cLocalBackUpPathFull, cLocalArchivePathFull, uiBackUpMinFreeSpaceFull, uiArchiveMinFreeSpaceFull);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки ХРАНИЛИЩА МЕДЛЕННОЙ записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"</form>\r\n",
				session->request_id,
				cnt, 
				cLocalBackUpPathSlow, cLocalArchivePathSlow, uiBackUpMinFreeSpaceSlow, uiArchiveMinFreeSpaceSlow);
	strcat(msg_tx, msg_subbody);	
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки ХРАНИЛИЩА записи ДВИЖЕНИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				...
				session->request_id,
				cnt, 
				cLocalBackUpPathDiff, cLocalArchivePathDiff, uiBackUpMinFreeSpaceDiff, uiArchiveMinFreeSpaceDiff);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки ХРАНИЛИЩА АУДИО записи:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				...
				session->request_id,
				cnt, 
				cLocalBackUpPathAudio, cLocalArchivePathAudio, uiBackUpMinFreeSpaceAudio, uiArchiveMinFreeSpaceAudio);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки ХРАНИЛИЩА СТАТУСОВ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				...
				session->request_id,
				cnt, 
				cLocalBackUpPathStatuses, cLocalArchivePathStatuses, uiBackUpMinFreeSpaceStatuses, uiArchiveMinFreeSpaceStatuses);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='LemonChiffon'><td>Настройки ХРАНИЛИЩА СОБЫТИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='LemonChiffon'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				...
				session->request_id,
				cnt, 
				cLocalBackUpPathEvents, cLocalArchivePathEvents, uiBackUpMinFreeSpaceEvents, uiArchiveMinFreeSpaceEvents);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody, 
				...
				"<input type='hidden' name='req_index' value=%i>\r\n"
				"<input type='hidden' name='Num' value=%i>\r\n"
				"<tr bgcolor='#d0cca7'><td>Настройки ХРАНИЛИЩА ДЕЙСТВИЙ:</td><td></td><td><button type='submit'>Сохранить</button></td></tr>"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам копий:</td><td><input type='text' name='Val1' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Путь к файлам архива:</td><td><input type='text' name='Val2' maxlength=255 value='%s' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для копий (MB):</td><td><input type='number' name='Val3' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				"<tr bgcolor='#d0cca7'><td>* Минимальное поддерживаемое место на диске для архива(MB):</td><td><input type='number' name='Val4' min=0 max=100000 value='%i' style='width: 300px;'></td></tr>\r\n"
				...
				session->request_id,
				cnt, 
				cLocalBackUpPathActions, cLocalArchivePathActions, uiBackUpMinFreeSpaceActions, uiArchiveMinFreeSpaceActions);
	strcat(msg_tx, msg_subbody);
	...
	sprintf(msg_subbody,
				...
				session->request_id,
				cnt, 
				cMailAuth, Copy_iEmailLog, LogMailAddress, Copy_uiLogEmailPauseSize / 1000, (char*)&Copy_uiSoundMessageID, Copy_uiSoundMessagePauseSize / 1000);
	strcat(msg_tx, msg_subbody);
	cnt++;
	memset(msg_subbody, 0, 3072);
	sprintf(msg_subbody,
