#!/usr/bin/perl -w
# Copyright &#169; 2007-2017 Jamie Zawinski <jwz@jwz.org>
#
...
my ($version) = ('$Revision: 1.1044 $' =~ m/\s(\d[.\d]+)\s/s);
...
sub load_youtube_formats($$$) {
  ...
  my $info_url_1 = ("https://www.youtube.com/get_video_info?video_id=$id" .
                    # Avoid the "playback restricted" error. This is a referer.
                    '&eurl=' . url_quote ($url) .
                    ($sts ? '&sts=' . $sts : ''));

  # Sometimes this arg is needed to avoid "blocked it from display
  ...
  ($title) = ($body =~ m@&title=([^&]+)@si) unless $title;
  errorI ("$id: no title in $info_url_1") if (!$title && $urlmap);
